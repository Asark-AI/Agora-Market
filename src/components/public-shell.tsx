...
export function PublicShell({ children }: { children: React.ReactNode }) {
  ...
  return (
    <div className="min-h-screen overflow-x-hidden bg-background text-foreground">
      ...
      <nav className="fixed inset-x-0 bottom-0 z-30 border-t border-border/70 bg-background/95 shadow-[0_-10px_30px_-20px_rgba(15,23,42,0.28)] backdrop-blur-xl md:hidden">
        <div className="mx-auto flex max-w-7xl items-center justify-between px-2 py-2 pb-[max(0.5rem,env(safe-area-inset-bottom))]">
          {mobileNavItems.map(({ href, label, icon: Icon }, index) => {
            ...
            return (
            <Link key={`${label}-${index}`} href={resolvedHref} className={`relative flex flex-1 flex-col items-center gap-1 rounded-2xl px-2 py-2 text-[11px] font-medium transition hover:bg-muted hover:text-foreground ${isActive ? 'text-primary' : 'text-muted-foreground'}`}>
