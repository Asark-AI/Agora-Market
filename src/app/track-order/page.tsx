  ...
  return (
    <main className="min-h-screen bg-slate-50 px-4 py-6 text-slate-950 sm:px-8 sm:py-10">
      <div className="mx-auto max-w-3xl space-y-5">
        <header className="flex items-center justify-between"><div><p className="text-sm font-medium text-slate-500">Agora</p><h1 className="text-2xl font-semibold tracking-tight">Track your order</h1></div><span className="rounded-md border border-amber-200 bg-amber-50 px-2 py-1 text-[10px] font-bold uppercase tracking-wider text-amber-800">Demo tracking</span></header>
        <section className="rounded-xl border border-slate-200 bg-white p-5 shadow-sm sm:p-6"><div className="flex items-start justify-between gap-4"><div><p className="text-xs text-slate-500">Order</p><h2 className="mt-1 text-xl font-semibold">#{state.order.id}</h2><p className="mt-1 text-sm text-slate-500">Agora Standard · GH₵20 delivery</p></div><div className="text-right"><p className="text-xs text-slate-500">Total</p><p className="mt-1 text-lg font-semibold">GH₵{state.order.total}</p></div></div><div className="mt-5"><DemoMap status={state.delivery.status} role="buyer" /></div></section>

        <section className="rounded-xl border border-slate-200 bg-white p-5 shadow-sm sm:p-6"><div className="flex items-center justify-between"><h2 className="font-semibold">Delivery progress</h2><span className="text-xs text-slate-500">{state.delivery.status.replaceAll('_', ' ')}</span></div><div className="mt-5 space-y-4">{timeline.map((step, index) => { const complete = index <= currentIndex; const active = index === currentIndex; return <div key={step.status} className="flex items-center gap-3"><div className={`flex size-7 shrink-0 items-center justify-center rounded-full ${complete ? 'bg-slate-950 text-white' : 'border border-slate-300 bg-white text-slate-400'}`}>{complete ? <Check className="size-4" /> : <Circle className="size-3" />}</div><span className={`text-sm ${active ? 'font-semibold text-slate-950' : complete ? 'text-slate-700' : 'text-slate-400'}`}>{step.label}</span>{active && <span className="ml-auto text-xs text-emerald-700">Current</span>}</div>; })}</div></section>

        <section className="grid gap-5 md:grid-cols-2"><div className="rounded-xl border border-slate-200 bg-white p-5"><div className="flex items-center gap-3"><div className="flex size-10 items-center justify-center rounded-full bg-slate-100"><Truck className="size-5" /></div><div><p className="text-xs text-slate-500">Your rider</p><p className="font-semibold">Kwame Mensah</p><p className="text-xs text-slate-500">Motorcycle · ★ 4.8</p></div></div><div className="mt-4 flex items-center gap-2 text-xs text-slate-600"><MapPin className="size-3.5" /> {delivered ? 'Delivered successfully' : 'Arriving in about 15 minutes'}</div></div><div className="rounded-xl border border-slate-200 bg-white p-5"><div className="flex items-center gap-2"><ShieldCheck className="size-4 text-emerald-600" /><h2 className="font-semibold">Delivery verification</h2></div><p className="mt-2 text-xs leading-5 text-slate-500">Give this code to your rider when they arrive. Delivery can only be completed after server verification in production.</p><div className="mt-4 flex items-center justify-between rounded-lg bg-slate-50 px-3 py-3"><span className="text-xs text-slate-500">Your demo OTP</span><strong className="text-lg tracking-[0.35em]">{DEMO_OTP}</strong></div></div></section>
